...
class TestDuplicityHelper:
    @pytest.mark.parametrize('backend,remote_backup_url,expected_destination', [
    ...
    def test_backup_cmd(self, duplicity_helper, backend, remote_backup_url, expected_destination):
    ...
    @pytest.mark.parametrize('user,password,expected_destination', [
        ('ubuntu', None, 'scp://ubuntu@some.host/backups/unit-mock-0'),
        ('ubuntu', 'somepass', 'scp://ubuntu:somepass@some.host/backups/unit-mock-0')
    ])
    def test_backup_cmd_user_password(self, duplicity_helper, user, password, expected_destination):
        backend = 'scp'
    ...
    @pytest.mark.parametrize('disable_encryption,gpg_public_key,private_ssh_key,expected_options', [
        (True, None, None, ['--no-encryption']),
        (None, 'GPGK3Y', None, ['--gpg-key=GPGK3Y']),
        (None, None, 'mykey', ['--ssh-options=-oIdentityFile=/root/.ssh/duplicity_id_rsa']),
        (True, 'GPGK3Y', 'mykey', ['--no-encryption', '--ssh-options=-oIdentityFile=/root/.ssh/duplicity_id_rsa']),
        (None, 'GPGK3Y', 'mykey', ['--gpg-key=GPGK3Y', '--ssh-options=-oIdentityFile=/root/.ssh/duplicity_id_rsa']),
    ])
    def test_backup_cmd_additional_options(
            self, duplicity_helper, disable_encryption, gpg_public_key, private_ssh_key, expected_options):
    ...
    @pytest.mark.parametrize('aws_secret_access_key,aws_access_key_id,encryption_passphrase', [
        ('key', 'key_id', 'mypass'),
        ('key', 'key_id', ''),
        ('', '', ''),
    ])
    ...
    def test_do_backup(self, mock_os, mock_subprocess, duplicity_helper, aws_secret_access_key,
                       aws_access_key_id, encryption_passphrase):
    ...
    @pytest.mark.parametrize('backup_frequency,expected_frequency', [
        ('hourly', '@hourly'),
        ('daily', '@daily'),
        ('weekly', '@weekly'),
        ('monthly', '@monthly'),
    ...
    @patch('builtins.open')
    def test_setup_backup_cron(self, mock_open, mock_templating, mock_os, duplicity_helper,
    ...
    @pytest.mark.parametrize('key_exists,path_exists,permission,written', [
        (True, True, 'a+', False),
        (True, False, 'w+', False),
        (False, True, 'a+', True),
        (False, False, 'w+', True),
